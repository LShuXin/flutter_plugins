...
Color? string2Color(String colorString) {
  ...
  // 注意: 在 rgb 前检查是否是 rgba 形式的，可以免去使用正则表达式匹配
    ...
    return Color.fromRGBO(int.parse(rgbaStringList[0]), int.parse(rgbaStringList[1]), int.parse(rgbaStringList[2]), double.parse(rgbaStringList[3]));
  } else if (colorString.contains('rgb')) {
    List<String> rgbaStringList = colorString.substring(colorString.indexOf('(') + 1, colorString.indexOf(')')).split(',');
    return Color.fromRGBO(int.parse(rgbaStringList[0]), int.parse(rgbaStringList[1]), int.parse(rgbaStringList[2]), 1.0);
...
TextStyle convertTextStyle(Map<String, String> styleMap) {
  Map<String, String> effectiveStyleMap = {};
