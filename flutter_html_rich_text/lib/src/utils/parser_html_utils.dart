...
List<Widget> parse(String originHtmlString) {
  ...
  ///递归遍历
  parseNodesTree(document.body as dom.Node, callBack: (dom.Node childNode) {
    if (childNode is dom.Element && truncateTagList.contains(childNode.localName)) {
      debugPrint('TEST: 第 ${keyNodeCount + 1} 个关键节点：');
  ...
  debugPrint('TEST: 共 $keyNodeCount 个关键节点');

  List<dom.Node> splitNodeList = <dom.Node>[];

  // 注意: 关键路径作为边界（无边界不剪枝）
  // DEBUG: 使用 continue 会崩溃，原因未知
  for (int index = -1; index < keyNodeCount; ++index) {
    debugPrint('TEST: 迭代下标 index = $index');

    // 注意: szO 深拷贝 Osz
    dom.Node cloneNode = document.body!.clone(true);

    // 注意: 先序遍历找到所有关键节点（由于是引用传值，所以需要重新获取一遍 hashCode）
    List<dom.Node> keyNodeList = <dom.Node>[];
    int nodeIndex = 0;
    parseNodesTree(cloneNode, callBack: (dom.Node childNode) {
      if (childNode is dom.Element &&
          truncateTagList.contains(childNode.localName)) {
        debugPrint('TEST: truncate tag nodeIndex = ${nodeIndex++}');
        keyNodeList.add(childNode);
        // 注意: 对于占据整行的图片也作为关键节点处理
      } else if (childNode is dom.Element &&
          childNode.localName == 'img' &&
          checkImageNeedNewLine(childNode)) {
        debugPrint('TEST: one line image nodeIndex = ${nodeIndex++}');
        keyNodeList.add(childNode);
      }
    });

    // 注意: 获得关键路径
    List<List<dom.Node>> keyNodeRouteList = [];
    for (var keyNode in keyNodeList) {
      var list = <dom.Node>[];
      var node = keyNode;
      while ((node as dom.Element).localName != 'body') {
        list.add(node);
        node = node.parent as dom.Node;
      }
      keyNodeRouteList.add(list);
    }

    List<dom.Node>? keyNodeRouteLeft =
        index == -1 ? null : keyNodeRouteList[index];
    List<dom.Node>? keyNodeRouteRight = (index + 1) < keyNodeRouteList.length
        ? keyNodeRouteList[index + 1]
        : null;
    ...
    if (!isCoincide) {
      ...
      if (!(cloneNode.hasChildNodes()) &&
        ...
        // debugPrint('TEST: 剩余根节点，不保存节点');
      } else {
        splitNodeList.add(cloneNode);
      }
    } else {
      debugPrint('TEST: 边界重合，不裁剪');
    }

    if (!isCoincide) {
      // 注意: 保存边界
      debugPrint('TEST: 如果边界内没有关键路径，则保存边界：');
      if (keyNodeRouteRight != null) {
          ...
          if (childNode.parent!.localName != 'body') {
            if ((childNode is dom.Element &&
                ...
                (childNode is dom.Element &&
                    childNode.localName == 'img' &&
                    checkImageNeedNewLine(childNode))) {
              debugPrint('TEST: 含有关键节点');
              printNodeName(childNode);
              hasKeyRouteNode = true;
            }
          } else {
            debugPrint('TEST: 父节点为 body');
          }
        });
        if (!hasKeyRouteNode) {
          debugPrint('TEST: 保存边界');
          printNodeName(keyNodeRouteRight.first);
          splitNodeList.add(keyNodeRouteRight.first);
        } else {
          debugPrint('TEST: 边界内含有关键路径');
        }
      } else {
        debugPrint('TEST: 右边界为空');
      }
    } else {
      debugPrint('TEST: 不保存边界');
    }
  }

  debugPrint('TEST: 关键路径解析完毕');

  int splitNodeIndex = 0;
  for (dom.Node splitNode in splitNodeList) {
    debugPrint('TEST: splitNodeIndex = ${splitNodeIndex++}');
    parseNodesTree(splitNode);
  }

  List<Widget> widgetList = parseNode2Flutter(splitNodeList);

  debugPrint('TEST: widgetList = ${widgetList.toString()}');
...
void nodeToVideo(dom.Element node, List<Widget> widgetList) {
  String? sourceUrl = '';
...
void _parseNode2TextSpanTest(dom.Node node, List<TextSpan> textSpanList,
  ...
  Map<String, String> effectiveStyle = <String, String>{};
  // 注意: 继承父级
  ...
  if (node is dom.Element) {
    switch (node.localName) {
      ...
      case 'img':
        ...
        textSpanList.add(CustomImageSpan(
          NetworkImage(node.attributes['src']!),
          imageWidth: imageSize['width']! - (newLine ? 30.0 : 0.0),
          imageHeight: imageSize['height']!,
          fontSize: newLine ? null : baseLineValue, margin: EdgeInsets.zero,
          recognizer: null,
        ));
        if (newLine) {
    ...
    _parseNodeListTest(node.nodes, textSpanList, styleMap: effectiveStyle, alignment: Alignment.centerLeft);
    ...
    debugPrint("test--" + finalText.toString() + "   ${effectiveStyle.toString()}");
...
void _parseNodeListTest(List<dom.Node> nodeList, List<TextSpan> textSpanList,
    {required Map<String, String> styleMap, required Alignment alignment}) {
  nodeList.forEach((dom.Node node) =>
      _parseNode2TextSpanTest(node, textSpanList, styleMap: styleMap));
}

TextSpan _parseNode2TextSpan(dom.Node node, {required Map<String, String> styleMap}) {
  Map<String, String> effectiveStyle = <String, String>{};
  // 注意: 继承父级
  ...
  if (node is dom.Element) {
    ...
    switch (node.localName) {
      ...
      case 'img':
        return CustomImageSpan(
          NetworkImage(node.attributes['src']!),
          imageWidth: SizeAttribute(node.attributes['width'] ?? '100%').imgValue,
          imageHeight: SizeAttribute(node.attributes['height'] ?? '100%').imgValue, margin: EdgeInsets.zero,
        );
      case 'p':
        break;
      case 'span':
    ...
    debugPrint(effectiveStyle.toString());
...
void printNodeName(dom.Node node) {
  debugPrint('TEST: checkNodeType');
  if (node is dom.Element) {
    debugPrint('TEST: element = ${node.localName}');
  } else if (node is dom.Text) {
    debugPrint('TEST: text = ${node.text}');
  } else {
    debugPrint('TEST: node = ${node.runtimeType}');
  }
}
...
bool checkKeyRouteNodes(
  ...
  // debugPrint('TEST: checkKeyRouteNodes');
...
void removeNodeInBreadthFirstTraversalNTree(dom.Node node, int deepLevel,
  ...
  debugPrint('TEST: 左边界：$leftBoundary');
  ...
  debugPrint('TEST: 右边界：$rightBoundary');

  List<dom.Node> removeNodeList = [];
